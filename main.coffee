...
exports.handler = (config) ->
	(req, res, next) ->
		...
		req_path = if url_q.pathname == '/' then 'index' else url_q.pathname.slice 1
		...
		if ('.html' == req_ext) or ('' == req_ext)
			...
			respond_compiled_text(fn_base, '.styl', '.css', 'text/css',
				compile_stylus)

		else if '.js' == req_ext
			fn_base = req_path.slice 0, -3
			respond_compiled_text(fn_base, '.coffee', '.js', 'text/javascript',
				compile_coffee)
